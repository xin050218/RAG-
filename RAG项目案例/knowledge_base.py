...
class KnowledgeBaseService(object):
    ...
    def __init__(self):
        ...
        self.spliter = RecursiveCharacterTextSplitter(
            ...
            chunk_overlap = config.chunk_overlap, #连续文本之间的字符重叠数量最大值
            separators=config.separators,       #自然段落划分的符号
            length_function=len,            #长度，使用python自带的len函数做长度统计的依据
        )    #文本分割器的对象
    ...
    def upload_by_str(self, data: str , filename):
            ...
            self.chroma.add_texts(           #内容就加载到向量库当中了
                #iterable -> list \tuple
                knowledge_chunks,
